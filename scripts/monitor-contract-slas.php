...
$pdo=Database::connection();$pdo->exec("INSERT INTO commercial_contract_worker_runs(worker_key,status) VALUES('sla_monitor','running')");$id=(int)$pdo->lastInsertId();try{$r=(new CommercialContractService($pdo))->monitorSlas();$pdo->prepare("UPDATE commercial_contract_worker_runs SET status='ok',records_processed=?,detail=?,completed_at=NOW() WHERE id=?")->execute([$r['breached'],json_encode($r),$id]);echo $r['breached']." breached contract(s).\n";}catch(Throwable $e){$pdo->prepare("UPDATE commercial_contract_worker_runs SET status='failed',detail=?,completed_at=NOW() WHERE id=?")->execute([$e->getMessage(),$id]);fwrite(STDERR,$e->getMessage()."\n");exit(1);}
